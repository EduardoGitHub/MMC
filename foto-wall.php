<?php
  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'send')) {
 	...
 	//if($customer_id > 0){
		...
		$tamanhoLar_op4 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoLarop4']);
		$tamanhoAlt_op4 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoAltop4']);
		$estilo_op4 = tep_db_prepare_input($HTTP_POST_VARS['estiloop4']);
		$enquiry_op4 = nl2br($HTTP_POST_VARS['enquiryop4']);
		$modelo = tep_db_prepare_input($HTTP_POST_VARS['modelo']);
		$quantidade = tep_db_prepare_input($HTTP_POST_VARS['quantftop4']);
		...
		if (tep_validate_email($email)) {
		  ...
		  tep_redirect(tep_href_link('envie-seus-arquivos.php', 'nome='.$nome.'&email='.$email.'&telefone='.$telefone.'&cep='.$cep.'&url=foto-wall.php'));
		} else {
		  $error = true;
...
<div id="container">
  ...
  <div id="mainContent">
      ...
      <?php echo tep_draw_form('foto-wall', tep_href_link('foto-wall.php', 'action=send'), 'post', 'enctype="multipart/form-data"'); ?>
