  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'banner':
      ...
      if (tep_db_num_rows($banner_query)) {
        ...
        if($HTTP_GET_VARS['ext']== 'yes')
        	tep_redirect($banner['banners_url']);
		else if($HTTP_GET_VARS['ext']== 'no')
			tep_redirect_banner($banner['banners_url']);
      }
      break;
	  
	  case 'parceiro':
      $banner_query = tep_db_query("select banners_url from " . TABLE_PARTNERS . " where banners_id = '" . (int)$HTTP_GET_VARS['goto'] . "'");
      if (tep_db_num_rows($banner_query)) {
        $banner = tep_db_fetch_array($banner_query);
        tep_update_banner_click_count($HTTP_GET_VARS['goto']);
