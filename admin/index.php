...
*/
  ...
  for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
	...
	$newline = "<graph xAxisName='Mês' numberPrefix='R$' yAxisName='Valores' decimalPrecision='1' formatNumberScale='0'>";
	  ...
	  $cont = 0;
	   $array_colors = array('AFD8F8','F6BD0F','8BBA00','FF8E46','008E8E','D64646','8E468E','588526','588526','B3AA00','008ED6','9D080D','A186BE');
		while ( $months = tep_db_fetch_array( $months_query ) ) {
			...
			$shipping_total_query = tep_db_query( "SELECT SUM( value ) AS total FROM orders_total ot, orders o WHERE ot.orders_id=o.orders_id AND year( o.date_purchased ) = " . date('Y') . " AND month( o.date_purchased ) = " . $months['m'] . "  AND ot.class = 'ot_shipping'" );
			$shipping_total = tep_db_fetch_array( $shipping_total_query );
			$tax_total_query = tep_db_query( "SELECT SUM( value ) AS total FROM orders_total ot, orders o WHERE ot.orders_id=o.orders_id AND year( o.date_purchased ) = " . date('Y') . " AND month( o.date_purchased ) = " . $months['m'] . "  AND ot.class = 'ot_tax'" );
		...
		$newline .= '<set name="'.substr(mesReferente($months['month']),0,3).'" value="'.$net_total['total'].'" color="'.$array_colors[$cont].'" />';
$cont++;
	  }
		$newline .= '</graph>';
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                  	<td>
						...
						<? echo renderChart("includes/charts/includes/FCF_Column3D.swf", "includes/charts/xml/sales.xml", "", "chart3", 830, 300); ?>
