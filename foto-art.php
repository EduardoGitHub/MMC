  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'send')) {
 	...
 	//if($customer_id > 0){
		...
		define('EMAIL_SUBJECT', ' :: ORÇAMENTO :: ' . STORE_NAME);
			...
			$tamanhoLar_op3 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoLarop3']);
			$tamanhoAlt_op3 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoAltop3']);
			$tema = tep_db_prepare_input($HTTP_POST_VARS['information1op3']);
			$ideia = tep_db_prepare_input($HTTP_POST_VARS['ideiaop3']);
			$estPessoal = tep_db_prepare_input($HTTP_POST_VARS['estilop']);
			$estArte = tep_db_prepare_input($HTTP_POST_VARS['estiloa']);
			$enquiry_op3 = nl2br($HTTP_POST_VARS['enquirypo3']);
		...
		if (tep_validate_email($email)) {
		  ...
		  tep_redirect(tep_href_link('envie-seus-arquivos.php', 'nome='.$nome.'&email='.$email.'&telefone='.$telefone.'&cep='.$cep.'&url=foto-art.php'));
		} else {
		  $error = true;
...
<div id="container">
  ...
  <div id="mainContent">
      ...
      <?php echo tep_draw_form('foto-art', tep_href_link('foto-art.php', 'action=send'), 'post', 'enctype="multipart/form-data"'); ?>
