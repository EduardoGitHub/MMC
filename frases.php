  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'send')) {
 	...
 	//if($customer_id > 0){
		...
		define('EMAIL_SUBJECT', ' :: ORÇAMENTO :: ' . STORE_NAME);
			...
			$tamanhoLar_op2 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoLarop2']);
			$tamanhoAlt_op2 = tep_db_prepare_input($HTTP_POST_VARS['tamanhoAltop2']);
			$cor_op2 = tep_db_prepare_input($HTTP_POST_VARS['corop2']);
			$frase_op2 = nl2br($HTTP_POST_VARS['fraseop2']);
			$enquiry_op2 = nl2br($HTTP_POST_VARS['enquiryop2']);
	...
	if (tep_validate_email($email)) {
		  ...
		  tep_redirect(tep_href_link('envie-seus-arquivos.php', 'nome='.$nome.'&email='.$email.'&telefone='.$telefone.'&cep='.$cep.'&url=frases.php'));
		} else {
		  $error = true;
...
<div id="container">
  ...
  <div id="mainContent">
      ...
      <?php echo tep_draw_form('frases', tep_href_link('frases.php', 'action=send'), 'post', 'enctype="multipart/form-data"'); ?>
