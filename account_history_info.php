          ...
          <tr>
            <td class="main" colspan="2">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td><b><?php echo sprintf(HEADING_ORDER_NUMBER, $HTTP_GET_VARS['order_id']) . ' <small>(' . $order->info['orders_status'] . ')</small>'; ?></b> </td>
					<td align="right">
						<? 
					//PEGA VALOR SEM R$
					...
					if (($order->info['orders_status'] != 'Entregue' or $order->info['orders_status'] != 'Enviado') and $order->info['payment_method'] == '<div style="float:left; margin:10px 5px 0 0">Boleto Bancário Banco do Brasil</div><div style="float:left"><img src="images/formas-de-pagamento-boleto.png" /></div><div style="clear:left"></div>') 
						...
						?>
							...
							<input type="hidden" id="order_id" name="order_id" value="<?php echo $HTTP_GET_VARS['order_id']; ?>"> 
						...
						<?php 
						  ...
						  $get_frete = tep_db_query("select value from ".TABLE_ORDERS_TOTAL." where class ='ot_shipping' and orders_id ='" . $HTTP_GET_VARS['order_id'] . "'");
				  		  $data_frete = tep_db_fetch_array($get_frete);
						 
						  $desconto = mysql_fetch_array(tep_db_query("select value from ".TABLE_ORDERS_TOTAL." where class ='ot_discount_coupon' and orders_id ='" . $HTTP_GET_VARS['order_id'] . "'"));						  
						  ?>
							...
							<input type="hidden" name="ref_transacao" value="Pedido: <?php echo $HTTP_GET_VARS['order_id']; ?> - Cliente: <?php echo  session_is_registered('customer_id') ; ?>">
