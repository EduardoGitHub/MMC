<?php
  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'send')) {
    ...
    $num_pedido = tep_db_prepare_input($HTTP_POST_VARS['numpedido']);
	$codeofbars = tep_db_prepare_input($HTTP_POST_VARS['codeofbars']);
  }
?>
  ...
  <span class="main"><?php echo TEXT_INFORMATION; ?></span><br /><br />
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" >
		  <?php
            ...
            if(CLIENT_ZEUS =='True')
                {
                    if(isset($HTTP_POST_VARS['numpedido']) && $HTTP_POST_VARS['numpedido'] <> '' && $HTTP_POST_VARS['purchase'] ==2){
                    ...
                    }else if(isset($HTTP_POST_VARS['numpedido']) && $HTTP_POST_VARS['numpedido'] <> '' && $HTTP_POST_VARS['purchase'] ==1){
                    ...
                    }else if(isset($HTTP_POST_VARS['codeofbars']) and $HTTP_POST_VARS['codeofbars'] <> '' && $HTTP_POST_VARS['purchase'] ==2){
                        ...
                        if(isset($HTTP_POST_VARS['numpedido']) && $HTTP_POST_VARS['numpedido'] <> ''){
                $query = mysql_query('SELECT o.date_purchased, op.products_name, p.products_warranty FROM orders o, orders_products op, products p WHERE o.orders_id = '.$num_pedido.' and op.orders_id ='.$num_pedido.' and p.products_id = op.products_id');
                        if(mysql_num_rows($query)>=1){
                            $imprimi .= '<tr bgcolor="#CCCCCC">
                                            <td width="185" class="classTitulos">&nbsp;PRODUTO</td>
