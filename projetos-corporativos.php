  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'send')) {
 	...
 	//if($customer_id > 0){
		...
		define('EMAIL_SUBJECT', ' :: ORÇAMENTO :: ' . STORE_NAME);
			...
			$nomeu = tep_db_prepare_input($HTTP_POST_VARS['nome']);
			$email = tep_db_prepare_input($HTTP_POST_VARS['email']);
			$telefone = tep_db_prepare_input($HTTP_POST_VARS['telefone']);
			$cep = tep_db_prepare_input($HTTP_POST_VARS['cep']);
			
			$erro_email = checa_email($email);
			if($erro_email == "") $sql = mysql_query("INSERT INTO newsletter(email,nome)VALUES('".$email."','".$nome."')");
			
			$nome = tep_db_prepare_input($HTTP_POST_VARS['nomeempresa']);
			$ramo = tep_db_prepare_input($HTTP_POST_VARS['ramosegmento']);
			$site = tep_db_prepare_input($HTTP_POST_VARS['site']);
			$sobre = tep_db_prepare_input($HTTP_POST_VARS['sobreempresa']);
			$publico = tep_db_prepare_input($HTTP_POST_VARS['publico']);
			$decorar = tep_db_prepare_input($HTTP_POST_VARS['decorar']);
			$ideia = tep_db_prepare_input($HTTP_POST_VARS['ideia']);
			$estPessoal = tep_db_prepare_input($HTTP_POST_VARS['estiloe']);
			$estArte = tep_db_prepare_input($HTTP_POST_VARS['estiloa']);
			//$cor = tep_db_prepare_input($HTTP_POST_VARS['corop1']);
			$obs = tep_db_prepare_input($HTTP_POST_VARS['enquiryop1']);
		...
		if (tep_validate_email($email)) {
		  ...
		  tep_redirect(tep_href_link('envie-seus-arquivos.php', 'nome='.$nomeu.'&email='.$email.'&telefone='.$telefone.'&cep='.$cep.'&url=projetos-corporativos.php'));
		} else {
		  $error = true;
...
<div id="container">
  ...
  <div id="mainContent">
      ...
      <?php echo tep_draw_form('projetos-corporativos', tep_href_link('projetos-corporativos.php', 'action=send'), 'post', 'enctype="multipart/form-data"'); ?>
