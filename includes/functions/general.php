...
if ($suburb != '') $streets = $street . $cr .$number. $cr . $complemento . $cr . $suburb; 
if ($country == '') $country = tep_output_string_protected($address['country']); 
if ($state != '') $statecomma = tep_putcaracters($zip, '#####-###').$cr.$state . ', ';
...
//                    e.g. info@mytepshop.com
  ...
  function tep_sendMail($to_email_address, $email_subject, $html, $from_email_name, $from_email_address){ //NOVO
	...
	<html>
		...
		<body>
		<table width="760" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #CCC; padding:15px; background-color:#D7AB00" align="center">
...
function tep_display_parcela($total) { 
	if(PARCELAMENTO_SHOW == 'true'){
		$splits = ((int)(($total)/PARCELAMENTO_CARTAO_MINIMO)); 
		$minimo = PARCELAMENTO_CARTAO_MINIMO * 2;
		if(PARCELAMENTO_CARTAO_SEM_JUROS == 1){
			if ($splits > PARCELAMENTO_CARTAO_PARCELAS){
				$splits = PARCELAMENTO_CARTAO_PARCELAS;
			}
		}else $splits = PARCELAMENTO_CARTAO_SEM_JUROS;
		$valor = '';
		if (($splits > 0)and($total >= $minimo )){
			if($splits <= PARCELAMENTO_CARTAO_SEM_JUROS){
				$taxa = 0;
				$total = $total/$splits;
				$valor = 'em até <span>'.$splits.'x</span> de <span>R$'. number_format($total, 2 , "," , ".").'</span> no cartão <b>sem juros</b>';
			}else{
				$fator = array('','1.00000','0.51875','0.35007','0.26575','0.21518','0.18148','0.15743','0.13941','0.12540','0.11420','0.10505','0.09743');
				$valor = 'em até <span>'.$splits.'x</span> de <span>R$'. number_format($total*($fator[$splits]), 2 , "," , ".") .'</span> no cartão';
			}
		}
		return($valor);
	}else
 ...
 function tep_discount_products($products_id){
		...
		$message = 'ou <span>R$'.number_format($discount_values['discount_new_products_price'], 2 , "," , ".").'</span> à vista com <span>' .round($discount_values['discount_percent'],0).'%</span> de desconto';
 ...
 function tep_discount_products_price($products_id, $newprice){
		...
		$message = 'ou <span>R$'.number_format($price, 2 , "," , ".").'</span> à vista com <span>' .round($discount_values['discount_percent'],0).'%</span> de desconto';
		}
		
		return $message;
}

function tep_make_catsf_ullist($rootcatid = 0, $maxlevel = 0, $param){
		$output = '';
   global $cPath_array, $show_full_tree, $languages_id;
		...
		$result = tep_db_query('select c.categories_id, cd.categories_name, c.parent_id from ' . TABLE_CATEGORIES . ' c, ' . TABLE_CATEGORIES_DESCRIPTION . ' cd where c.categories_id = cd.categories_id '.$param.' and categories_status = 1 and cd.language_id="' . (int)$languages_id .'" '.$parent_query.' order by sort_order, cd.categories_name');
