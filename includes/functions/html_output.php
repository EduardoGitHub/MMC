...
// Creates a pull-down list of countries
  ...
  function tep_display_option($product_id, $wicth_option, $languages_cod, $required = false, $price = 0) {
		...
		if($products_options_num_rows > 0){//Verifica se tem algum atributo para esse registro
			...
			$field = '<div style="font-family:Tahoma; font-size:12px; color:#000; font-weight:bold; text-align:center; padding:5px;"><img src="images/detalheProdutoInfo.jpg" />'.$products_options_name['products_options_name'].' '.$t.'</div>';//Mostra o nome da opção
			...
			while ($products_options = tep_db_fetch_array($products_options_query)) {
				...
				if ($products_options['options_values_price'] != '0'){
					...
					$products_options_array[sizeof($products_options_array)-1]['re'] .= 'R$'.number_format($preco_size,2,',','.');
					$products_options_array[sizeof($products_options_array)-1]['ret'] .= $preco_size;
					$products_options_array[sizeof($products_options_array)-1]['rett'] .= '<span class="textdesconto" style="color:#666; font-size:14px"><br />'.tep_display_parcela($preco_size).'</span>';
					$products_options_array[sizeof($products_options_array)-1]['rettt'] .= '<span class="textdesconto" style="color:#666; font-size:14px">'.tep_discount_products_price($product_id, $preco_size).'</span>';
				}else{
					$products_options_array[sizeof($products_options_array)-1]['re'] .= 'R$'.number_format($price,2,',','.');	
					$products_options_array[sizeof($products_options_array)-1]['ret'] .= $price;
					$products_options_array[sizeof($products_options_array)-1]['rett'] .= '<span class="textdesconto" style="color:#666; font-size:14px"><br />'.tep_display_parcela($price).'</span>';
					$products_options_array[sizeof($products_options_array)-1]['rettt'] .= '<span class="textdesconto" style="color:#666; font-size:14px">'.tep_discount_products_price($product_id, $price).'</span>';
				}
			}

		if (isset($cart->contents[$HTTP_GET_VARS['products_id']]['attributes'][$products_options_name['products_options_id']]))  $selected_attribute = $cart->contents[$HTTP_GET_VARS['products_id']]['attributes'][$products_options_name['products_options_id']];
			...
			if($wicth_option == 1){	
				...
				for ($i=0; $i<$n; $i++) {
				  ...
				  $field .= '<div class="optionTamanho" id="size'.$i.'" >
					...
					$field .= '</div>
								<div class="info"><span>Tamanho do desenho: <br><b>'.$values[$i]['obs'].'</b></span><br /><span><input type="hidden" id="hpred'.$i.'" name="hpre'.$i.'" value="'.$values[$i]['ret'].'"/><span id="textparc'.$i.'" style="display:none;">'.$values[$i]['rett'].'</span> <span id="textdescavista'.$i.'" style="display:none;">'.$values[$i]['rettt'].'</span></span>
								</div>
								<div class="valor"><span id="valor'.$i.'" style="font-weight:bold;">'.$values[$i]['re'].'</span></div>
								<div style="clear:both"></div>
								<div class="desc">'.$values[$i]['desc'].'</div>
							</div>';					
				}
				...
				$paginas = array('lona.html','canvas.html','reposicionavel.html');
				$field .= '<div class="optionMidiaImpressao"><ul>';
				for ($i=0, $n=sizeof($values); $i<$n; $i++) {	
					
					$field .= '<li><p><input type="radio" name="' . tep_output_string($name) . '" value="' . tep_output_string($values[$i]['id']) . '" onclick="ValorMidiaCm('.$values[$i]['ret'].')"/><label>'.tep_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')).' '.tep_output_string($values[$i]['obs']).' <a style="color:#0000FF; cursor:pointer" onclick="javascript:popupMidias(\''.$paginas[$i].'\')"> + Detalhes</a></label> </p></li>';
				}
				$field .= '</ul></div>';	
