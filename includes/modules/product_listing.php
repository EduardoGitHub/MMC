...
$categories_query = tep_db_query("SELECT categories_name, categories_description FROM categories_description WHERE categories_id = ".$_GET['cPath']);
$categories_dados =  tep_db_fetch_array($categories_query)
?>
 
<div class="TextoCategorias">
	<h1><?=$categories_dados['categories_name']?></h1><br />
    <div><?=$categories_dados['categories_description'];?></div>
</div>
<?php
}
  ...
  if ($listing_split->number_of_rows > 0) {
	...
	for ($range=0; $range<sizeof($price_ranges); $range++) {
		...
		if (isset($HTTP_GET_VARS['manufacturers_id']))	
			$sortby .= tep_hide_session_id().tep_draw_hidden_field('manufacturers_id', $_GET['manufacturers_id']).tep_draw_hidden_field('keywords', $_GET['keywords']);
		else
			$sortby .= tep_hide_session_id().tep_draw_hidden_field('cPath', $_GET['cPath']).tep_draw_hidden_field('keywords', $_GET['keywords']);
    ...
    echo '<div class="pagestitulo"><div class="CaminhoNav" style="position:absolute; left:0; top:10px;"><b>Você está em:</b> </b><a href="'.tep_href_link(FILENAME_DEFAULT).'">Home</a> / '.$breadcrumb->trail(' / ').'</div><span style="position:absolute; right:10px;">'.$sortby.' </span></div>';
...
<a class="addthis_counter addthis_pill_style"></a>
	  ...
	  if ( ($listing_split->number_of_rows > 0) && ( (PREV_NEXT_BAR_LOCATION == '1') || (PREV_NEXT_BAR_LOCATION == '3') ) ) {
		...
		echo '<div style="padding-top:10px;"><span class="smallText">'.$listing_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS).'</span> <span style="float:right" class="smallText">'.TEXT_RESULT_PAGE . ' ' . $listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))).'</span></div>';
  }
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_PRICE':
            ...
            if(tep_not_null($listing['specials_new_products_price'])) {
			  ...
			  if($not_show =='False'){	 //VERIFICA SE OS PREÇOS E O BOTÃO DE COMPRA PODE SER EXIBIDO  (SHOW_STORE_FOR_USERS )	
              	if(!isset($customer_revenda)||($customer_revenda==0))//VERIFICANDO TIPO DE CONSUMIDOR
				$lc_text = 'por: '.$currencies->display_price($listing['products_price'],  tep_get_tax_rate($listing['products_tax_class_id'])) . '<br /><span class="textdesconto">' . tep_display_parcela($listing['products_price']).' '.tep_discount_products($listing['products_id']).'</span>';
   ...
   if (tep_db_num_rows($categories_query) > 0) {
	...
	echo '<li style="width:'.$widthli.';">
			<div style="max-width:380px; margin:0 auto;">
				<div style="width:'.SMALL_IMAGE_WIDTH.'px; height:'.SMALL_IMAGE_HEIGHT.'px;">'.$product_contents[0].'</div><br/>
				<strong><b>'.$product_contents[1].'</b></strong><br/>
				'.$preco.'
